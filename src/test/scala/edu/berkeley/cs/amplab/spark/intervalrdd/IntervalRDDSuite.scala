...
class IntervalRDDSuite extends ADAMFunSuite with Logging {
    ...
    sparkTest("assert the data pulled from a file is the same") {
      ...
      val r2 = new ReferenceRegion("chrM", 0L, 100L)
      val r3 = new ReferenceRegion("chrM", 300L, 400L)

      val r4 = new ReferenceRegion("chrM", 2000L, 3000L)
      val r5 = new ReferenceRegion("chrM", 2300L, 2400L)
      ...
      var results = intRDD.get(r2, "person1")
      var results2 = intRDD.get(r3, "person1")
      var results3 = intRDD.get(r5, "person1")

      println("FIRST REGION GET")
      println(results.get.head._2.size)
      println(results2.get.head._2.size)
      println(results3.get)


      var secondLoad = sc.loadIndexedBam(bamFile, r4)
      val secondData = Array((r4, ("person1", secondLoad.collect.toList)))
      val secondRdd = sc.parallelize(secondData)
      var intRDD2 = intRDD.multiput(secondRdd, dict)

      results = intRDD2.get(r2, "person1")
      results2 = intRDD2.get(r3, "person1")
      results3 = intRDD2.get(r5, "person1")

      println("SECOND REGION GET")
      println(results.get.size)
      println(results.get.size)
      println(results.get.head._1)
      println(results2.get.head._1)
      println(results.get.head._2.size)
      println(results2.get.head._2.size)
      println(results3.get.head._2.size)
